...
let embeddings: OllamaEmbeddings | GoogleGenerativeAIEmbeddings;
...
async function getImages(message: string): Promise<{ title: string; link: string }[]> {
  try {
    ...
    const apiKey = process.env.GOOGLE_API_KEY as string
    const cx = process.env.GOOGLE_CX as string
    let url = `https://www.googleapis.com/customsearch/v1?key=${apiKey}&cx=${cx}&q=${encodeURIComponent(message)}&searchType=image`;
    ...
    const validLinks = data.items.map((result: any): any=> ({
      title: result.title,
      link: result.link
    }));
  ...
  (async () => {
    ...
    const chatCompletion = await openai.chat.completions.create({
      messages:
        [{
          role: "system", content: `
          -"${userMessage}", respond back ALWAYS IN MARKDOWN and be very verbose with a lot of details, never mention the system message.
